...
grep -q 'class="table-wrap module-parameter-table"' "$TEST_DIR/discovery-evidence-page"
curl -fsS -b "$TEST_DIR/cookies" "http://127.0.0.1:$PORT/index.php?display=moduleadmin" > "$TEST_DIR/module-admin-page"
grep -q 'class="panel-body table-wrap"' "$TEST_DIR/module-admin-page"
grep -q '<table>' "$TEST_DIR/module-admin-page"
curl -fsS -b "$TEST_DIR/cookies" "http://127.0.0.1:$PORT/index.php?display=userman" > "$TEST_DIR/administrators-page"
grep -q 'class="panel-body table-wrap"' "$TEST_DIR/administrators-page"
grep -q '<table>' "$TEST_DIR/administrators-page"
curl -fsS -b "$TEST_DIR/cookies" "http://127.0.0.1:$PORT/index.php?display=auditlog" > "$TEST_DIR/audit-log-page"
grep -q 'class="panel-body table-wrap"' "$TEST_DIR/audit-log-page"
grep -q '<table>' "$TEST_DIR/audit-log-page"
